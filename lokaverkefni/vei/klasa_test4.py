...
if __name__ == "__main__":
    ...
    x_train, x_test, psi_train, psi_test = train_test_split(x_vals, psi_vals, test_size=0.2, random_state=42)
    ...
    # Define learning rates to be used
    eta_values = [0.0001,0.001,0.01, 1 / eigen[-1]]

    # Train model for each learning rate and plot loss
    ...
    for eta in eta_values:
        ...
        # Plot the loss for the current learning rate
        if eta == eta_values[-1]:
            plt.plot(losses.numpy(), label=fr"Training Loss $\left(\eta=\frac{{1}}{{\lambda_\mathrm{{max}}}}\right)$")
        else:
            plt.plot(losses.numpy(), label=fr"Training Loss ($\eta$={eta:.4f})")
